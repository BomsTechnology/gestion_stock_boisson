<template>
    ...
    <section class="max-w-4xl p-6 mx-auto bg-white dark:bg-gray-800">
        <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Ajouter une Boisson</h2>
            ...
            <div class="grid grid-cols-1 gap-6 mt-4">
              ...
              <div>
                ...
                <div class="mt-1 flex rounded-md shadow-sm">
                  ...
                  <input v-model="form.price" type="number" name="price" id="price" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300">
                </div>
              </div>

              <div>
                <label class="text-gray-700 dark:text-gray-200" for="quantity">Quantité de cassier</label>
                <input v-model="form.quantity" id="quantity" type="number" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                </div>
...
export default {
    setup(){
        ...
        const { createDrink, errors } = useDrinks();

        const storeDrink = async () => {
