    ...
    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
      <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200" v-for="output in filteredOutputs" :key="output.id">            
            <tr>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <a :href="'outputs/' + output.id + '/details'" class="px-2 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-700"> Détails </a >
                <!-- <router-link :to="{name: 'outputs.details', params: { id: output.id }}" class="px-2 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-700"> Détails </router-link > -->
...
export default {
    ...
    setup(){
        const {outputs, getOutputs } = useOutputs();
        return{
            outputs,
            getOutputs,
        }
  ...
  computed: {
    filteredOutputs() {
       let filteredByCustomer =  this.outputs.filter((output) => {
            return output.customer.toLowerCase().includes(this.searchKey.toLowerCase());
        })
        ...
        if(filteredByCustomer.length != 0){
          return filteredByCustomer;
            ...
            return output.id.toLowerCase().includes(this.searchKey.toLowerCase());
        });
        }
    },
  },
