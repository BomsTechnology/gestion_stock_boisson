    ...
    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
      <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200" v-if="moves.length != 0">            
            <tr v-for="move in moves" :key="move.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <button @click="details(move)" class="px-2 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-700"> Détails </button>
              </td>
            </tr>
...
export default {
  ...
  setup(props) {
    ...
    const { getTransactionCustomer, moves, loading} = useDashboard();
    const { checkNotification, notifications} = useNotification();
    onMounted(getTransactionCustomer(props.id), checkNotification());
  ...
  methods: {
    details(move){
      window.location.assign('/outputs/' + move.id + '/details');  
    },
    toogleNotif() {
