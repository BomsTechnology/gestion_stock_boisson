...
export default {
  ...
  setup(props) {
    ...
    const { getTransactionProvider, moves, loading} = useDashboard();
    const { checkNotification, notifications} = useNotification();
    onMounted(getTransactionProvider(props.id), checkNotification());
  ...
  methods: {
    details(move){
      window.location.assign('/inputs/' + move.id + '/details');  
    },
    toogleNotif() {
